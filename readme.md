...
| :----------------------- | :------------------------------------ | :--------------------------------------------------------------------------------------------------- | :------------------------------------------ | :--------------------------------------- |
| **MCU**                  |Mikroprocesor(Microcontroller Unit)| Integrovaný čip (CPU + RAM + Flash na jednom substrátu), deterministický běh bez OS nebo RTOS        | např. ESP32, PIC16LF1xxx, RP2040            |Domácí spotřebiče, dálkové ovladače, hračky, automobilová elektronika, zařízení internetu věcí|
| **MPU**                  |Microprocesor(Microprocessor Unit)| Samostatný procesor vyžadující externí RAM a úložiště, zpravidla běží plnohodnotný OS (Linux)        | např. STM32F103, ATmega328P, ESP32|Chytré telefony, osobní počítače, Wi-Fi router, multimediální systémy v automobilech nebo pokročilé síťové prvky|
| **Embedded**             |Embedded PLC (Embedded Programmable Logic Controller)|klasický PLC automat integrovaný přímo na desku plošných spojů |např. B&R X20, S7-1500, Velocio | Bílá technika, bankomaty, regulace kotlů |
| **PLC**                  |programovatelný logický automat(Programmable Logic Controller)| Průmyslový automat pro cyklické deterministické řízení procesů, vysoká odolnost, modulární/kompaktní |např. S7-1200, ControlLogix, Modicon M241, M251, M580|Detekce balíku, Třídění|
| **iPC**                  |Průmyslové PC(Industrial PC)|osobní počítač upravený pro provoz v drsném průmyslovém prostředí |např. IPC-A-610 |Velíny továren, řízení výrobních linek v automobilkách, kamerové systémy pro kontrolu kvality (Machine Vision) nebo energetické uzly|
| **Programovatelné relé** |Programovatelné relé (Smart Relays)| Zjednodušené kompaktní PLC pro méně náročné úlohy, nahrazuje časovací relé a stykačové kombinace | např. Siemens LOGO!, Zelio Logic, easyE4, ZEN|automatické otevírání bran a žaluzií, řízení osvětlení budov, ovládání bazénové filtrace, jednoduché dopravníkové pásy nebo zavlažovací systémy|
...
| **Pořizovací cena HW na 1 kus** *(nízká < 500 Kč / střední 5–30 tis. Kč / vysoká > 50 tis. Kč)*       | **Střední** *(cca 3 500 – 6 000 Kč)*                                                                                                                                                                                      | `nízká`                         | `středení`                     | `vysoká`                                |
